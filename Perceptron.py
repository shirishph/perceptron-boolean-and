class Perceptron(object):
    ...
    def train(self):
        ...
        for epoch in range(self.epochs):
            print("------------------------------------------------------------------------")
            labels_lower_row = ["x1", "x2", "LR", "t", "sum", "act", "dw1", "dw2", "db", "w1", "w2", "b"]
            ...
            for row in self.truth_table:
                ...
                cells = [row[0], row[1], self.learning_rate, row[2], sum, activate]

                change = self.weight1 + self.learning_rate * (row[2] - activate) * row[0]
                cells.append(change)
                self.weight1 = change

                change = self.weight2 + self.learning_rate * (row[2] - activate) * row[1]
                cells.append(change)
                self.weight2 = change

                change = self.bias + self.learning_rate * (row[2] - activate)
                cells.append(change)
                self.bias = change
